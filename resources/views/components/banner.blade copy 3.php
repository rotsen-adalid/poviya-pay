...
<div x-data="{ shown: false, timeout: null, style: {{$style}} }"
    x-data="{{ json_encode(['show' => false, 'style' => $style, 'timeout' => null]), }}"
    x-init="@this.on('{{ $on }}', () => { clearTimeout(timeout); shown = true; timeout = setTimeout(() => { shown = false }, 3000);  })"
    x-show.transition.opacity.out.duration.1500ms="shown"
    style="display: none;"
    class=" fixed w-full top-12 bg-opacity-100"
